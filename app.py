...
# [중요] 공공데이터포털 인증키(서비스키) — 코드에 직접 넣지 않고 '비밀값'으로 관리합니다.
#   - 인증키를 GitHub 등에 그대로 올리면 노출되어 도용될 수 있습니다.
#   - 그래서 아래처럼 st.secrets 에서 먼저 읽고, 없으면 환경변수에서 읽습니다.
#
#   [로컬 실행]  프로젝트 안에  .streamlit/secrets.toml  파일을 만들고 아래처럼 적으세요:
#       SERVICE_KEY = "여기에_발급받은_Decoding_키"
#   [클라우드 배포]  Streamlit Community Cloud 앱 설정의 'Secrets' 칸에 같은 내용을 붙여넣으세요.
#
#   - 인증키는 Decoding 키(순수 글자/숫자)를 권장합니다. requests 가 자동으로 인코딩합니다.
import os  # 환경변수 대체 경로용

def _load_service_key():
    """st.secrets > 환경변수 순서로 인증키를 읽습니다. 없으면 빈 문자열."""
    try:
